...
async function addToProcessing(tx) {
    ...
    const alreadyRegisteredSignature = dbs.sequencerSigToNumber.get(decoded.ledgerPosition);
    ...
    if (afterActionsGas < 1) {
        console.error("Not enough gas to execute actions for transaction " + currentTxN);
        executionCoreChannel.postMessage({ sender: "system", actions: [], gas: 0, id: currentTxN ,hash:null});
        return;
    }
    

    executionCoreChannel.postMessage({ sender: decoded.address, actions: decoded.actions, gas: afterActionsGas, id: decoded.ledgerPosition,hash:decoded.txID() });
}