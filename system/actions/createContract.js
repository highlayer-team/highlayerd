...
module.exports = {
    ...
    async execute(action, { highlayerNodeState, dbs, interaction, actionNumber, macroTasks }) {
        ...
        const contractId = bech32.encode("hlcontract", 0, crypto.createHash("blake2s256").update(Buffer.concat([Buffer.from(interaction.hash, "hex"), Buffer.from(numberToPaddedHex(actionNumber), "hex"), Buffer.from(sourceId, "hex")])).digest())
        await dbs.contracts.put(contractId, sourceId)
        macroTasks.addToPriority({
            sender: contractId, actions: initActions, gas: gasForInitActions, hash: crypto.createHash("blake2s256").update(contractId + interaction.hash + numberToPaddedHex(actionNumber)).digest("hex")
        })
    }
}